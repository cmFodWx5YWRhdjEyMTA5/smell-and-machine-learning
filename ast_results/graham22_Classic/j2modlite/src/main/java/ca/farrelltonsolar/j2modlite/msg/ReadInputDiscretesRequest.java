// isComment
/**
 * isComment
 */
package ca.farrelltonsolar.j2modlite.msg;

import java.io.DataInput;
import java.io.DataOutput;
import java.io.IOException;
import ca.farrelltonsolar.j2modlite.Modbus;
import ca.farrelltonsolar.j2modlite.ModbusCoupler;
import ca.farrelltonsolar.j2modlite.procimg.DigitalIn;
import ca.farrelltonsolar.j2modlite.procimg.IllegalAddressException;
import ca.farrelltonsolar.j2modlite.procimg.ProcessImage;

/**
 * isComment
 */
public final class isClassOrIsInterface extends ModbusRequest {

    // isComment
    private int isVariable;

    private int isVariable;

    /**
     * isComment
     */
    public isConstructor() {
        super();
        isMethod(isNameExpr.isFieldAccessExpr);
        /*isComment*/
        isMethod(isIntegerConstant);
    }

    /**
     * isComment
     */
    public isConstructor(int isParameter, int isParameter) {
        super();
        isMethod(isNameExpr.isFieldAccessExpr);
        // isComment
        isMethod(isIntegerConstant);
        isMethod(isNameExpr);
        isMethod(isNameExpr);
    }

    // isComment
    /**
     * isComment
     */
    public ReadInputDiscretesResponse isMethod() {
        ReadInputDiscretesResponse isVariable = new ReadInputDiscretesResponse(isMethod());
        isNameExpr.isMethod(isMethod());
        isNameExpr.isMethod(isMethod());
        isNameExpr.isMethod(isMethod());
        if (!isMethod()) {
            isNameExpr.isMethod(isMethod());
            isNameExpr.isMethod(isMethod());
        }
        return isNameExpr;
    }

    public ModbusResponse isMethod() {
        ReadInputDiscretesResponse isVariable = null;
        DigitalIn[] isVariable = null;
        // isComment
        ProcessImage isVariable = isNameExpr.isMethod().isMethod();
        // isComment
        try {
            isNameExpr = isNameExpr.isMethod(isMethod(), isMethod());
        } catch (IllegalAddressException isParameter) {
            return isMethod(isNameExpr.isFieldAccessExpr);
        }
        isNameExpr = isMethod();
        /*isComment*/
        for (int isVariable = isIntegerConstant; isNameExpr < isNameExpr.isFieldAccessExpr; isNameExpr++) isNameExpr.isMethod(isNameExpr, isNameExpr[isNameExpr].isMethod());
        return isNameExpr;
    }

    /**
     * isComment
     */
    public void isMethod(int isParameter) {
        if (isNameExpr < isIntegerConstant || isNameExpr + isNameExpr >= isIntegerConstant)
            throw new IllegalArgumentException();
        isNameExpr = isNameExpr;
    }

    /**
     * isComment
     */
    public int isMethod() {
        return isNameExpr;
    }

    /**
     * isComment
     */
    public void isMethod(int isParameter) {
        if (isNameExpr < isIntegerConstant || isNameExpr > isIntegerConstant || isNameExpr + isNameExpr >= isIntegerConstant)
            throw new IllegalArgumentException();
        isNameExpr = isNameExpr;
    }

    /**
     * isComment
     */
    public int isMethod() {
        return isNameExpr;
    }

    public void isMethod(DataOutput isParameter) throws IOException {
        isNameExpr.isMethod(isNameExpr);
        isNameExpr.isMethod(isNameExpr);
    }

    public void isMethod(DataInput isParameter) throws IOException {
        isNameExpr = isNameExpr.isMethod();
        isNameExpr = isNameExpr.isMethod();
    }

    public byte[] isMethod() {
        byte[] isVariable = new byte[isIntegerConstant];
        isNameExpr[isIntegerConstant] = (byte) ((isNameExpr >> isIntegerConstant) & isIntegerConstant);
        isNameExpr[isIntegerConstant] = (byte) ((isNameExpr & isIntegerConstant));
        isNameExpr[isIntegerConstant] = (byte) ((isNameExpr >> isIntegerConstant) & isIntegerConstant);
        isNameExpr[isIntegerConstant] = (byte) ((isNameExpr & isIntegerConstant));
        return isNameExpr;
    }
}
