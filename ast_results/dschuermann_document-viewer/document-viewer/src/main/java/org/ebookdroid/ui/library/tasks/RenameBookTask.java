...
public class isClassOrIsInterface extends BaseFileAsyncTask<BookNode, FileTaskResult> {
    ...
    protected final FileUtils.FilePath isVariable;

    protected BookNode isVariable;

    protected File isVariable;

    public isConstructor(final Context isParameter, final RecentAdapter isParameter, final FileUtils.FilePath isParameter) {
    ...
    protected FileTaskResult isMethod(final BookNode... isParameter) {
        ...
        try {
            File isVariable = isMethod(this.isFieldAccessExpr.isMethod());
            if (isNameExpr != null) {
